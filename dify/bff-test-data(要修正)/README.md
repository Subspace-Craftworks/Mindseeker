...
- `action-cases/`
  ...
  - Each fixture contains both the BFF chat request and the expected Dify tool/action output.
...
Use the `bff_request.body.query` text to drive Dify through the BFF, then compare the invoked tool/action against `expected_tool_call`.

Important:

- `bff_request` is what the BFF sends to Dify.
- `expected_tool_call` is what Dify should emit as the selected tool/action.
- Do not treat `expected_tool_call` as a payload that the BFF sends directly.

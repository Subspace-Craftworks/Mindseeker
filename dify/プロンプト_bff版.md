...
**JSON出力のルール:**
* 【必須】`current_goal_id` は必ず出力してください。ユーザーが現在フォーカスしているゴールのIDを指定します。話題が新しいゴールに移った場合や、新しく作成した場合は必ず `"NEW"` と指定してください。（BFFが直後に作成した新しいゴールのIDへ自動解決します）
* 上記の `operations` はあくまで「利用可能な操作の一覧」です。実際には**必要な操作のみ**を抽出して `operations` 配列に出力してください。操作が不要な場合は `operations` を空配列 `[]` にしてください。
* 新しく親要素（GoalやSubjectなど）を **同じ `operations` 配列内で `create_xxx` アクションを使って作成した場合のみ**、その直後に紐づける子要素の親IDとして `"NEW"` を指定できます。勝手に `"NEW"` を指定して親要素が自動生成されることはありません。（例：`create_subject` と `create_issue` を同時に発行し、`create_issue` の `subject_id` に `"NEW"` を指定するなど）

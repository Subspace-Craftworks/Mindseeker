-- Update get_goal_detail to sort issues and tasks by subject order then created_at ascending
  ...
  SELECT json_build_object(
    ...
    'issues', COALESCE((
      ...
      FROM (
        ...
        ORDER BY sub.updated_at DESC, i.created_at ASC
      ) i
    ), '[]'::json),
    'tasks', COALESCE((
      ...
      FROM (
        ...
        ORDER BY sub.updated_at DESC, t.created_at ASC
      ) t
    ), '[]'::json),
    'events', COALESCE((
